#!/bin/bash
# deploy-master.sh — Deploy Ray Head and Coder services (Master Node)
# Usage: bash deploy-master.sh
# ============================================================
set -e
...
echo "=== Cleaning up legacy model checkpoints to free disk space ==="
...
echo "=== Deploying start-ray-head.sh ==="
...
echo "  ✅ /home/<username>/start-ray-head.sh"

echo "=== Deploying download-model.sh ==="
...
echo "  ✅ /home/<username>/download-model.sh"

echo "=== Deploying /home/<username>/vllm.env ==="
cat > /home/<username>/vllm.env << 'EOF'
# vLLM Coder Service Configuration
VLLM_MODEL=Qwen/Qwen3-Coder-Next-FP8
VLLM_SERVED_MODEL_NAME=Qwen3-Coder-Next-FP8
VLLM_MAX_MODEL_LEN=131072
EOF
echo "  ✅ /home/<username>/vllm.env"

echo "=== Deploying /etc/systemd/system/ray-head.service ==="
...
EOF
echo "  ✅ /etc/systemd/system/ray-head.service"

echo "=== Deploying /etc/systemd/system/vllm-coder.service ==="
...
ExecStartPre=+/bin/bash -c 'echo "Clearing Page Cache (RAM)..."; sync && sysctl -w vm.drop_caches=3'
...
WantedBy=multi-user.target
SCRIPT
echo "  ✅ /etc/systemd/system/vllm-coder.service"
...
echo ""
echo "Done. To start Master Node Services:"
echo "  1. Start Ray Head: sudo systemctl restart ray-head.service"
echo "  2. Start Coder (Port 8000): sudo systemctl restart vllm-coder.service"
echo ""
echo "Note: Make sure to deploy and start ray-worker.service on the Worker node so it can join the cluster."